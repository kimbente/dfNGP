...
if TRACK_EMISSIONS_BOOL:
    ...
    tracker = EmissionsTracker(project_name = "dfGPcm_real_experiments", output_dir = MODEL_REAL_RESULTS_DIR)
    tracker.start()
...
for region_name in ["region_lower_byrd", "region_mid_byrd", "region_upper_byrd"]:
    ...
    # NOTE: This part is different for dfGP
    # Calculate mean vector once, to intialise constant mean
    mean_vector = y_train.mean(dim = 0)
    ...
    for run in range(NUM_RUNS):
        ...
        # NOTE: we use a multitask likelihood for the dfGPcm model but with a global noise term
